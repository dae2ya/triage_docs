...
| `--xlsx` | CSV 대신 엑셀(`.xlsx`)로 저장. **`요약` 시트**(유형별 분포·파서별 집계·업로드불가/대용량/손상 목록 = 콘솔 요약 그대로) + **`상세` 시트**(파일별 판정 표) 2장. 요약 시트에는 **서식**(제목·헤더 배경·강조색·열너비)과 **파이차트 3종**(파서별 비율·형식별 개수·상태 구성)이 함께 들어갑니다. 외부 라이브러리 없이 표준 라이브러리로 생성 |
...
요약 시트는 제목·헤더 배경, 업로드불가·손상(빨강)·대용량(주황) 강조색, 열너비가 적용되고,
시트 하단의 차트데이터 블록을 원본으로 **파이차트 3종**(파서별 비율·형식별 개수·상태 구성)이
그려집니다. 항목이 1개뿐인 차트(예: 형식이 한 종류)는 자동으로 생략됩니다.
