...
| 이유 | 판정 근거(단서 수치 포함) |
| 용량 | MB 단위 (예: `73.9MB`) |
| 용량_추천 | 20 MB↑면 줄일 방법 안내, 이하면 빈칸 (안내 문구일 뿐 자동 압축은 하지 않음) |
...
| 2 | 중복 | `_duplicates` |
| 3 | 대용량(20 MB↑) | `_large` |
| 4 | 그 외 원본·고유 | `LLM_Parser` · `Docling` · `검토` |

- 중복은 **SHA-256 내용 해시**로 판정합니다(파일명이 아니라 내용 기준).
- 대용량은 파서 폴더보다 먼저 `_large`로 빠집니다 — 파싱 전에 용량부터 줄이라는 뜻입니다. **파일을 자동으로 압축하지는 않습니다**(재인코딩이 오히려 용량을 키우는 경우가 있어 분리만 하고, 축소는 사람이 판단).
